<?php

function loadEnv($path) {
    if (!file_exists($path)) {
        die(".env file not found at $path");
    }

    $lines = file($path, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);

    foreach ($lines as $line) {
        if (strpos(trim($line), '#') === 0)
            continue;

        if (!str_contains($line, '='))
            continue;

        list($name, $value) = explode('=', $line, 2);

        $name = trim($name);
        $value = trim($value);

        putenv("$name=$value");
        $_ENV[$name] = $value;
    }
}

loadEnv(__DIR__ . '/.env');

function getEnvVar($key) {
    return $_ENV[$key] ?? getenv($key);
}

function jiraPostRequest($url, $payload) {
    $email = getenv('JIRA_EMAIL');
    $token = getenv('JIRA_API_TOKEN');

    $ch = curl_init($url);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_POST => true,
        CURLOPT_POSTFIELDS => json_encode($payload),
        CURLOPT_HTTPHEADER => [
            "Authorization: Basic " . base64_encode("$email:$token"),
            "Accept: application/json",
            "Content-Type: application/json"
        ]
    ]);

    $response = curl_exec($ch);
    curl_close($ch);

    return json_decode($response, true);
}

function jiraGetRequest($url) {
    $email = getenv('JIRA_EMAIL');
    $token = getenv('JIRA_API_TOKEN');

    $ch = curl_init($url);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_HTTPHEADER => [
            "Authorization: Basic " . base64_encode("$email:$token"),
            "Accept: application/json"
        ]
    ]);

    $response = curl_exec($ch);
    curl_close($ch);

    return json_decode($response, true);
}

function getWorklogs($from, $to, $accountIds = []) {
    try {

        // Normalize input (string → array)
        if (is_string($accountIds)) {
            $accountIds = array_filter(explode(',', $accountIds));
        }

        if (empty($accountIds)) {
            return [
                'logs' => [],
                'total' => 0,
                'error' => 'accountIds are required'
            ];
        }

        // Cache key (important for multi users)
        $cacheKey = "worklogs_{$from}_{$to}_" . md5(json_encode($accountIds));

        if ($cached = cache($cacheKey)) {
            return $cached;
        }

        $results = [];
        $totalSeconds = 0;

        $fromDate = strtotime($from);
        $toDate   = strtotime($to . ' 23:59:59');

        // 🔥 Build JQL for multiple users
        $quotedIds = array_map(fn($id) => "\"$id\"", $accountIds);
        $authorJql = implode(',', $quotedIds);

        $jql = "worklogAuthor IN ($authorJql) 
                AND worklogDate >= \"$from\" 
                AND worklogDate <= \"$to\"";

        $base = getEnvVar('JIRA_BASE_URL');
        $searchUrl = "$base/rest/api/3/search/jql";

        $data = jiraPostRequest($searchUrl, [
            "jql" => $jql,
            "maxResults" => 50,
            "fields" => ["summary"]
        ]);

        if (!isset($data['issues']) || !is_array($data['issues'])) {
            return [
                'logs' => [],
                'total' => 0,
                'error' => 'Invalid Jira response'
            ];
        }

        foreach ($data['issues'] as $issue) {

            $key = $issue['key'] ?? null;
            if (!$key) continue;

            $summary = $issue['fields']['summary'] ?? 'No summary';

            $worklogsUrl = "$base/rest/api/3/issue/$key/worklog";
            $worklogsData = jiraGetRequest($worklogsUrl);

            if (!isset($worklogsData['worklogs'])) continue;

            foreach ($worklogsData['worklogs'] as $log) {

                if (!isset($log['started'], $log['timeSpentSeconds'])) continue;

                $authorId = $log['author']['accountId'] ?? '';

                // 🔥 MULTI USER FILTER
                if (!in_array($authorId, $accountIds)) continue;

                $started = strtotime($log['started']);
                if (!$started || $started < $fromDate || $started > $toDate) continue;

                $seconds = (int) $log['timeSpentSeconds'];
                $totalSeconds += $seconds;

                // ADF comment parsing
                $comment = '';
                if (!empty($log['comment']['content'])) {
                    foreach ($log['comment']['content'] as $block) {
                        if (!empty($block['content'])) {
                            foreach ($block['content'] as $c) {
                                if (isset($c['text'])) {
                                    $comment .= $c['text'] . ' ';
                                }
                            }
                        }
                    }
                }

                $results[] = [
                    'key' => $key,
                    'summary' => $summary,
                    'date' => date('Y-m-d', $started),
                    'user' => $log['author']['displayName'] ?? '',
                    'hours' => round($seconds / 3600, 2),
                    'comment' => trim($comment) ?: '-'
                ];
            }
        }

        $final = [
            'logs' => $results,
            'total' => round($totalSeconds / 3600, 2)
        ];

        cache($cacheKey, $final, 300);

        return $final;

    } catch (\Exception $ex) {
        return [
            'logs' => [],
            'total' => 0,
            'error' => $ex->getMessage()
        ];
    }
}
function searchUsers($query) {

    if (strlen($query) < 3) {
        return [];
    }

    $cacheKey = "user_search_" . md5($query);

    if ($cached = cache($cacheKey)) {
        return $cached;
    }

    $base = getEnvVar('JIRA_BASE_URL');

    $url = "$base/rest/api/3/user/search?query=" . urlencode($query);

    $data = jiraGetRequest($url);

    if (!is_array($data)) {
        return [];
    }

    $results = [];

    foreach ($data as $user) {

        if (!isset($user['accountId']))
            continue;

        $results[] = [
            'accountId' => $user['accountId'],
            'name' => $user['displayName'] ?? '',
            'email' => $user['emailAddress'] ?? ''
        ];
    }

    // Limit results (important for UI + API load)
    $results = array_slice($results, 0, 10);

    cache($cacheKey, $results, 300);

    return $results;
}

function cache($key, $data = null, $ttl = 300) {
    $file = __DIR__ . '/cache_' . md5($key) . '.json';

    if ($data === null) {
        if (file_exists($file) && (time() - filemtime($file)) < $ttl) {
            return json_decode(file_get_contents($file), true);
        }
        return null;
    }

    file_put_contents($file, json_encode($data));
}

function clearCache() {
    foreach (glob(__DIR__ . '/cache_*.json') as $file) {
        unlink($file);
    }
}
